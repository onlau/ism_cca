...
from src.utils.config import config, constants
...
def INTENSITY(f, T, N0, paths):
    base_intensity = PLANCK_FUNCTION(f, T)
...
def PHOTOIONIZATION_PER_FREQUENCY(f, T, N0, paths):
    ...
    ionization_rate = bound_free_coefficient * INTENSITY(f, T, N0, paths) / (constants.h * f) * np.pi * 4 * 100
